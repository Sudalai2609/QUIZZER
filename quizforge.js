...
const AIExtractor = {
  ...
  init(keys) {
    this.keys = keys;
    this.available = keys.openrouter.filter(Boolean);
  },
...
${text}`;
  ...
  async extractChunk(text) {
    for (let i = 0; i < this.available.length; i++) {
      const key = this.available[(this._robin + i) % this.available.length];
      try {
        const raw = await this.callOpenRouter(text, key);
        this._robin++;
        const parsed = this.parseJSON(raw);
        this._lastLog = '[openrouter] got ' + parsed.length + ' items. Raw: ' + (raw ? raw.slice(0,400) : 'null');
        return parsed;
      } catch(e) {
        this._lastLog = '[openrouter] ERROR: ' + e.message;
      }
    }
    return [];
  },
...
const UI = {
  ...
  promptAPIKeys() {
    return new Promise(resolve => {
      ...
      Object.assign(scrollBox.style, { display:'flex', flexDirection:'column', gap:'.5rem', maxHeight:'320px', overflowY:'auto' });
      inputs.openrouter = [];
      for (let i = 0; i < 20; i++) {
        const wrap = document.createElement('div');
        Object.assign(wrap.style, { display:'flex', flexDirection:'column', gap:'.2rem' });
        const lbl = document.createElement('label');
        lbl.textContent = 'OpenRouter Key ' + (i + 1);
        Object.assign(lbl.style, { fontSize:'.7rem', color:'#5b6a85', textTransform:'uppercase', letterSpacing:'.05em' });
        const inp = document.createElement('input');
        inp.type = 'password';
        inp.placeholder = i === 0 ? 'Required — paste sk-or-... key' : 'Optional — paste sk-or-... key';
        Object.assign(inp.style, {
          background:'#1a2236', border:'1px solid rgba(255,255,255,.08)', borderRadius:'8px',
          padding:'.55rem .9rem', color:'#f0f4ff', fontFamily:'monospace', fontSize:'.78rem',
          width:'100%', boxSizing:'border-box'
        });
        inputs.openrouter.push(inp);
      ...
      goBtn.onclick = (e) => {
        e.stopPropagation();
        const keys = {
          openrouter: inputs.openrouter.map(inp => inp.value.trim()).filter(Boolean)
        };
        modal.remove();
        resolve(keys);
      };
  ...
  async startProcessing() {
    ...
    if (!AIExtractor.available.length) {
      ...
      alert('Please enter at least one OpenRouter key.');
      return;
    }
